...
```tsx
useComments(editor: Editor | null): Comments
```

Reads the `CommentThreadStore` configured through `createBlockKit({ comment: { store } })`,
subscribes to store changes, and exposes the selection-driven composer state. See the
[comments guide](/docs/guides/comments).
